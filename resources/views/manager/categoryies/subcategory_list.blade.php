							...
							<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
								...
								<div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center me-3 flex-wrap mb-5 mb-lg-0 lh-1">
									...
									<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
										...
										<!--end::Item-->
										<!--begin::Item-->
										<li class="breadcrumb-item text-dark">Subcategory List'i</li>
										<!--end::Item-->
									</ul>
									<!--end::Breadcrumb-->
								</div>
									...
									<div class="card-header border-0 pt-6">
										...
										<div class="modal-dialog modal-dialog-centered mw-750px">
											...
											<div class="modal-header">
												<h5 class="modal-title">Yeni Subcategory</h5>
												<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Kapat"></button>
											</div>

											<div class="modal-body">
												<form action="{{ route('subcategory.store') }}" method="POST" id="add_subcategory_form">
													@csrf
													@if ($errors->any() && old('_token') && request()->routeIs('subcategory.store'))
													...
													<div class="mb-3">
														<label for="add_category_id" class="form-label">Kateqoriya <span class="text-danger">*</span></label>
														<select class="form-select @error('category_id') is-invalid @enderror" id="add_category_id" name="category_id" required>
															<option value="">Kateqoriya seçin</option>
															@foreach($categories as $cat)
																<option value="{{ $cat->id }}" {{ old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name_az }}</option>
															@endforeach
														</select>
														@error('category_id')
													...
													<div class="mb-3">
														<label for="add_name_az" class="form-label">Ad (Azərbaycan) <span class="text-danger">*</span></label>
														<input type="text" class="form-control @error('name_az') is-invalid @enderror" id="add_name_az" name="name_az" value="{{ old('name_az') }}" required>
														@error('name_az')
															<div class="invalid-feedback">{{ $message }}</div>
														@enderror
													</div>
													...
													<div class="mb-3">
														<label for="add_name_en" class="form-label">Ad (English)</label>
														<input type="text" class="form-control" id="add_name_en" name="name_en" value="{{ old('name_en') }}">
													</div>

													<div class="mb-3">
														<label for="add_name_ru" class="form-label">Ad (Русский)</label>
														<input type="text" class="form-control" id="add_name_ru" name="name_ru" value="{{ old('name_ru') }}">
													</div>

													<div class="mb-3">
														<label for="add_slug_az" class="form-label">Slug (Azərbaycan)</label>
														<input type="text" class="form-control @error('slug_az') is-invalid @enderror" id="add_slug_az" name="slug_az" value="{{ old('slug_az') }}" placeholder="Avtomatik yaradılacaq">
														@error('slug_az')
															<div class="invalid-feedback">{{ $message }}</div>
														@enderror
													</div>
													...
													<div class="mb-3">
														...
														<input type="text" class="form-control @error('slug_en') is-invalid @enderror" id="add_slug_en" name="slug_en" value="{{ old('slug_en') }}" placeholder="Avtomatik yaradılacaq">
														@error('slug_en')
															<div class="invalid-feedback">{{ $message }}</div>
														@enderror
													</div>
													...
													<div class="mb-3">
														...
														<input type="text" class="form-control @error('slug_ru') is-invalid @enderror" id="add_slug_ru" name="slug_ru" value="{{ old('slug_ru') }}" placeholder="Avtomatik yaradılacaq">
														@error('slug_ru')
															<div class="invalid-feedback">{{ $message }}</div>
														@enderror
													</div>
											...
											<div class="modal-footer">
												...
												<button type="submit" form="add_subcategory_form" class="btn btn-primary">Əlavə et</button>
											</div>
										...
										<table class="table  align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table" role="grid">
											...
											<tbody  id="not_search"   class="fw-bold text-gray-600">
												...
												<tr>
                                                    ...
                                                    <td>
                                                        <a href="#" class="text-dark fw-bold">{{ $category->category->name_az ?? '-' }}</a>
                                                    </td>
													...
													<td class="text-end">
														...
														<div class="menu-item px-3">
															...
															<div class="menu-item px-3">
																<a href="{{ route('subcategory.delete', $category->id) }}" class="menu-link px-3 delete-link">Sil</a>
															</div>
												...
												<!--begin::Edit Modal for {{ $category->id }}-->
												<div class="modal fade" id="edit_modal_{{ $category->id }}" tabindex="-1" aria-hidden="true">
												<div class="modal-dialog modal-dialog-centered mw-750px">
													<div class="modal-content">

													<div class="modal-header">
														<h5 class="modal-title">Subcategory Redaktə</h5>
														<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Bağla"></button>
													</div>

													<div class="modal-body">
														<form action="{{ route('subcategory.update', $category->id) }}" method="POST" id="edit_form_{{ $category->id }}">
															@csrf
															@method('PUT')

															<div class="mb-3">
																<label for="edit_category_id_{{ $category->id }}" class="form-label">Kateqoriya <span class="text-danger">*</span></label>
																<select class="form-select" id="edit_category_id_{{ $category->id }}" name="category_id" required>
																	<option value="">Kateqoriya seçin</option>
																	@foreach($categories as $cat)
																		<option value="{{ $cat->id }}" {{ $category->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name_az }}</option>
																	@endforeach
																</select>
															</div>

															<div class="mb-3">
																<label for="edit_name_az_{{ $category->id }}" class="form-label">Ad (Azərbaycan) <span class="text-danger">*</span></label>
																<input type="text" class="form-control" id="edit_name_az_{{ $category->id }}" name="name_az" value="{{ $category->name_az }}" required>
															</div>

															<div class="mb-3">
																<label for="edit_name_en_{{ $category->id }}" class="form-label">Ad (English)</label>
																<input type="text" class="form-control" id="edit_name_en_{{ $category->id }}" name="name_en" value="{{ $category->name_en }}">
															</div>

															<div class="mb-3">
																<label for="edit_name_ru_{{ $category->id }}" class="form-label">Ad (Русский)</label>
																<input type="text" class="form-control" id="edit_name_ru_{{ $category->id }}" name="name_ru" value="{{ $category->name_ru }}">
															</div>

															<div class="mb-3">
																<label for="edit_slug_az_{{ $category->id }}" class="form-label">Slug (Azərbaycan)</label>
															...
															<div class="mb-3">
																<label for="edit_slug_en_{{ $category->id }}" class="form-label">Slug (English)</label>
															...
															<div class="mb-3">
																<label for="edit_slug_ru_{{ $category->id }}" class="form-label">Slug (Русский)</label>
															...
															<div class="mb-3">
																<label for="edit_position_{{ $category->id }}" class="form-label">Position</label>
													...
													<div class="modal-footer">
														<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Ləğv et</button>
														<button type="submit" form="edit_form_{{ $category->id }}" class="btn btn-primary">Yadda saxla</button>
													</div>

												</div>
												</div>
												</div>
												<!--end::Edit Modal for {{ $category->id }}-->
