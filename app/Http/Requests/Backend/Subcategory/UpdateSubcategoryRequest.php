...
class UpdateSubcategoryRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Slug unique validation (cari subcategory-ni istisna edirik)
            if (Subcategory::where('slug_az', $slugAz)->where('id', '!=', $subcategoryId)->exists()) {
                $validator->errors()->add('slug_az', 'Bu slug (Azərbaycan) artıq istifadə olunur.');
            }

            if ($slugEn && Subcategory::where('slug_en', $slugEn)->where('id', '!=', $subcategoryId)->exists()) {
                $validator->errors()->add('slug_en', 'Bu slug (English) artıq istifadə olunur.');
            }

            if ($slugRu && Subcategory::where('slug_ru', $slugRu)->where('id', '!=', $subcategoryId)->exists()) {
    ...
    {
        // Slug-ları hazırlayırıq və merge edirik
        $this->merge([
